const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/teca-chat`;
...
export async function streamTecaChat({
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 402) {
        onError("Ops, meus tentáculos se enrolaram 🐙 Créditos insuficientes. Adicione créditos ao workspace.");
        return;
      }
