...
const LoginPage: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Sucesso navega para fora da página (redirect ao Google) — não há
      // estado de "sucesso" local para tratar aqui.
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        lockedUntil
          ? `Muitas tentativas. Tente novamente em ${Math.ceil(LOCKOUT_MS / 1000)}s.`
          : err instanceof Error ? err.message : 'Erro ao autenticar. Verifique seus dados.'
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="glass-card p-8 w-full max-w-md animate-slide-up" style={{ animationDelay: '0.12s' }}>
          {view === 'welcome' && (
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl font-semibold text-foreground border transition-colors hover:bg-secondary/50 disabled:opacity-60 flex items-center justify-center gap-2"
                style={{ borderColor: 'hsl(var(--brand-purple) / 0.5)' }}
              >
                {!googleLoading && (
          ...
          {view === 'login' && (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {isLocked && (
                <p className="text-sm text-amber-300 bg-amber-400/10 rounded-lg px-3 py-2">
                  Muitas tentativas. Tente novamente em {remainingSeconds}s.
                </p>
              )}
              <button type="submit" disabled={loading || isLocked} className="w-full py-3.5 rounded-xl font-semibold text-primary-foreground btn-primary-glow disabled:opacity-60">
